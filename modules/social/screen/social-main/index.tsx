            ...
            return (
              <div key={v.heading.title}>
                <div className="mb-4 mt-8">
                  <p className="font-bold text-sm">{v.heading.title}</p>
                  <p className="text-[10px]">{v.heading.description}</p>
                </div>
                {v.room.map((r) => {
                  return (
                    <div key={r.title} className="rounded-lg bg-[#1c6758d9] p-4 text-white mb-4">
                      <p className="font-semibold text-[10px] mb-1">Live</p>
                      <p className="text-xs font-semibold mb-2">{r.title}</p>
                      <div className="flex items-center">
                        <div className="flex mb-1 mr-1">
                          {Array.from(new Array(3).keys()).map((j, k) => (
                            <div
                              className={`w-[16px] h-[16px] rounded-full bg-slate-200 ${
                                k > 0 && 'ml-[-7px]'
                        ...
                        <p className="text-[9px]">{r.participants} partisipan</p>
                      </div>
                      <p className="text-[10px]">Host Oyin Saratu</p>
                    </div>
                  );
                })}
              </div>
